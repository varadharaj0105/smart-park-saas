...
async function seedPSGAdmin() {
  try {
    const email = "23i369@psgtech.ac.in";
    ...
    await pool.query(`
      ...
      VALUES ('PSG Tech Super Admin', $1, $2, 'super_admin', 1, 'Platform')
      ON CONFLICT (email) 
      DO UPDATE SET role = 'super_admin', tenant_id = 1
    `, [email, hash]);
    
    console.log("PSG Tech Super Admin seeded successfully.");
  } catch (error) {
